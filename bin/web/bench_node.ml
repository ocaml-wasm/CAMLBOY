...
let run_rom_path rom_path frames =
  let rom_file =
    if Array.length Sys.argv > 1 then
      ...
      "resource/games/" ^ rom_path
  in
  let data = In_channel.(with_open_bin rom_file input_all) in
  let rom = Bigstringaf.of_string ~off:0 ~len:(String.length data) data in
...
let main () =
  ...
  let frames =
    if Array.length Sys.argv > 2 then int_of_string Sys.argv.(2) else 15000 in
  (* Load initial rom *)
